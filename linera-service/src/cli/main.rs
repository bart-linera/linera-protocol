...
async fn kill_all_processes(
    pids: &[u32],
    join_set: &mut JoinSet<Result<(Option<u32>, process::ExitStatus), anyhow::Error>>,
) {
    for &pid in pids {
        ...
        ClientCommand::MultiBenchmark {
                ...
                children.into_iter().zip(stdout_handles).zip(stderr_handles)
            {
                join_set.spawn(async move {
                    ...
                    stdout_handle.await?;
                    stderr_handle.await?;
                    Ok::<_, anyhow::Error>((child.id(), status))
                });
            }
            ...
            loop {
                tokio::select! {
                        ...
                        match result {
                            Some(Ok(Ok((pid, status)))) => {
                                if !status.success() {
                                    ...
                                    kill_all_processes(&children_pids, &mut join_set).await;
                    ...
                    _ = shutdown_notifier.cancelled() => {
                        info!("Shutdown signal received, stopping all benchmark processes");
                        kill_all_processes(&children_pids, &mut join_set).await;
                        break;
                    }
                }
